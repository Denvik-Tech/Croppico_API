...
def updateUI():
    ...
    flash_arg = ("avrdude", "-v", "-p", "ATmega4809", "-c", "arduino", "-P", "/dev/ttyUSB0", "-b", "115200", "-D", "-U",
                 "flash:w:/home/pi/flash.hex:i")
    flash = subprocess.call('%s %s %s %s %s %s %s %s %s %s %s %s %s' % flash_arg, shell=True)
