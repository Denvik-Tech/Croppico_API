...
class SlaveController(Thread):
    ...
    def getcalibResult(self):
        ...
        if float(isCalibdone) == 1:
            if logToFile: self.logger.info("returning true")
    ...
    def setLightStatus(self, light, state):
        ...
        self.lastUserInputTime = now()
        if logToFile: self.logger.info(">> Pressing light ")
        # while self.isComBusy: pass
        if self.isComBusy: self.__ser.flushInput()
        self.checkSlaveStatus()
    ...
    def setNewSettings(self, stype, params):
        ...
        if stype == 1:
            ...
            # result = self.pollSlave(self.__slave_requests.Settings.set_ph_levels(float(params["phmin"])*10, float(params["phmax"])*10, float(params["phideal"])*10, float(params["phsensor_setpoint"])*10, float(params["phpump_setpoint"])*10))
            result = self.pollSlave(self.__slave_requests.Settings.set_ph_levels(float(params["phmin"])*10, float(params["phmax"])*10, float(params["phideal"])*1000, float(params["phsensor_setpoint"])*10, float(params["phpump_setpoint"])*1000))
            ...
            result = self.pollSlave(self.__slave_requests.Settings.set_ec_levels(params["ecmin"], params["ecideal"], params["ecsensor_setpoint"], params["ecpump_setpoint"]))
            ...
            if params["solution"] == "ec_a_pump":
                ...
                if solutionValue:  # send entered value
                    ...
                    # return result
                # else:  # turn on pump
                #     print("ec a pump turned ON for calibration")
                #     result = self.pollSlave(
                #         self.__slave_requests.Settings.turn_on_ec_a_pump())
                #     if logToFile: self.logger.info("ec a pump turns on" + str(result))
                    # return result
            if params["solution"] == "ec_b_pump":
                ...
                if solutionValue:  # send entered value
                    ...
                    # return result
                # else:  # turn on pump
                #     print("ec b pump turned ON for calibration")
                #     result = self.pollSlave(
                #         self.__slave_requests.Settings.turn_on_ec_b_pump())
                #     if logToFile: self.logger.info("ec b pump turns on" + str(result))
                    # return result
    ...
    def setMaintenanceControl(self, mtype, state):
        ...
        # light-brightness
            ...
            print(self.peripherals.supplement_ec_a_pump)
            result = self.pollSlave(self.__slave_requests.Maintenance.set_ec_a_pump(state))
            if logToFile: self.logger.info(self.maintenanceevent.ec_a_pump(state))
            self.connectionHandler.publish(self.messages.event,
                                           json.dumps({"event": self.maintenanceevent.ec_a_pump(state)}))
            self.peripheralsMaintenanceState[self.peripherals.supplement_ec_a_pump] = bool(state)

        elif mtype == 8:
            print(self.peripherals.supplement_ec_b_pump)
            result = self.pollSlave(self.__slave_requests.Maintenance.set_ec_b_pump(state))
            if logToFile: self.logger.info(self.maintenanceevent.ec_b_pump(state))
            if logToFile: self.logger.info("maintanence mtype check" + str(mtype))
            self.connectionHandler.publish(self.messages.event,
                                           json.dumps({"event": self.maintenanceevent.ec_b_pump(state)}))
            self.peripheralsMaintenanceState[self.peripherals.supplement_ec_b_pump] = bool(state)
        if not state:
        ...
        try:
            ...
            if cmType == "settings":
                ...
                if cmMsg["ph_thresholds"]:
                    ...
                    cmMsg["ph_thresholds"]["phideal"] = float(0.2)
                    cmMsg["ph_thresholds"]["phsensor_setpoint"] = float(0.5)
                    cmMsg["ph_thresholds"]["phpump_setpoint"] = float(0.25)
                ...
                if cmMsg["ec_thresholds"]:
                    ...
                    cmMsg["ec_thresholds"]["ecmin"] = int((float(cmMsg["ec_thresholds"]["ecideal"])))
                    cmMsg["ec_thresholds"]["ecideal"] = int(float(100))
                    cmMsg["ec_thresholds"]["ecsensor_setpoint"] = int(float(100))
                    cmMsg["ec_thresholds"]["ecpump_setpoint"] = int(float(25))
        ...
        if "ec_pump" in cmMsg:
            if len(cmMsg["ec_pump"].split(":")) == 2:
                ...
                if ecptime is not 0:
                    ...
                    ect.start()
        if "ec_a_pump" in cmMsg:
            if len(cmMsg["ec_a_pump"].split(":")) == 2:
                ecaptime = int(cmMsg["ec_a_pump"].split(":")[0]) * 60 + int(cmMsg["ec_a_pump"].split(":")[1])
                if logToFile:
                    self.logger.info("EC_A-pump turned ON for - " + str(ecaptime) + "seconds")
                if ecaptime is not 0:
                    self.setMaintenanceControl(self.peripherals.getCode(self.peripherals.supplement_ec_a_pump), True)
                    ecat = Timer(ecaptime, self.setMaintenanceControl,
                                 (self.peripherals.getCode(self.peripherals.supplement_ec_a_pump), False))
                    ecat.start()
        if "ec_b_pump" in cmMsg:
            if len(cmMsg["ec_b_pump"].split(":")) == 2:
                ecbptime = int(cmMsg["ec_b_pump"].split(":")[0]) * 60 + int(cmMsg["ec_b_pump"].split(":")[1])
                if logToFile:
                    self.logger.info("EC_B-pump turned ON for - " + str(ecbptime) + "seconds")
                if ecbptime is not 0:
                    check = self.peripherals.getCode(self.peripherals.supplement_ec_b_pump)
                    if logToFile: self.logger.info("maintanence adhoc check" + str(check))
                    self.setMaintenanceControl(self.peripherals.getCode(self.peripherals.supplement_ec_b_pump), True)
                    ecbt = Timer(ecbptime, self.setMaintenanceControl,
                                 (self.peripherals.getCode(self.peripherals.supplement_ec_b_pump), False))
                    ecbt.start()
